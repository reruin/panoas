package {
	...
	public class WallMap extends Sprite
	{
		private var allpic = [];
		...
		private function startPlugin(e:Event = null):void
		{
			...
			EIF.trace((new Date).toLocaleTimeString()+" : Loading WallMap Plugin(Ver 1.20140516) Success ... ");
		...
		private function init():void
		{
			...
			leftBtn.alpha = rightBtn.alpha = backBtn.alpha = mapBtn.alpha = 0.5;
		...
		private function regListeners():void
		{
			...
			mapBtn.addEventListener(MouseEvent.MOUSE_OVER,overBtnHandler);
			...
			mapBtn.addEventListener(MouseEvent.MOUSE_OUT,outBtnHandler);
			...
			mapBtn.addEventListener(MouseEvent.CLICK,clickMapBtnHandler);
		}
		...
		private function drawMapBtn():Sprite
		{
			var t = new Sprite();
			t.buttonMode = true;
			t.addChild( new Bitmap(new MapBMD()));
			return t;
		}
		...
		{
			stop();
			EIF.call("navToggle" , 0);
		}
		
		private function clickMapBtnHandler(e):void
		{
			EIF.call("navToggle");
		}
