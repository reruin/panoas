package {
	...
	public class NavMini extends Sprite
	{
		...
		private function startPlugin(e:Event = null):void
		{
			...
			if(EIF.ready) 
			{
				EIF.trace((new Date).toLocaleTimeString()+" : Loading NavMini(User) Plugin Success ... ");
		...
		private function dragSprite_mouseMoveHandler(e):void
		{
			...
			_focus.position = nodes[getCloseNode()].position;
			refresh(_focus);//EIF.trace(_focus.position.toString())
			_focus.visible = true;
		}
		
		private var to:String;
		private function getCloseNode():String
		{
			var n = 1 , c = "";
			for(var i in nodes)
			{
				var d:Number = Geometry.distance(nodes[i].position , fromViewToNormalise( new Point(this.mouseX , this.mouseY)) );
