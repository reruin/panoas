 ...
 * it has modified for using plugins
 **/
package pano.utils {
 ...
 */	 
	public class TweenNano {
		...
		public function TweenNano(target:Object, duration:Number, vars:Object) {
			...
			this.autoAlpha = ("autoAlpha" in vars)?true:false;
			
			if(this.autoAlpha) {
				vars.alpha = vars.autoAlpha;
		...
		public function renderTime(time:Number):void {
			...
			if(this.autoAlpha) this.target.visible = (this.target.alpha!=0);
		}
...
//---- STATIC FUNCTIONS -------------------------------------------------------------------------
		...
		public static function repeat(target:Object,duration:Number,vars:Object){
			var loopNum = 100;
			duration *= loopNum;
			
			for (var p:String in vars) {
				if (!(p in _reservedProps)) {
					if(typeof(vars[p]) == "number")  vars[p] *= loopNum; //[property, start, change]
				}
		...
		public static function killTweens(complete:Boolean=false):void {
			for (var target in _masterList) {
				TweenNano.killTweensOf(_masterList[target] , true);
			}
		}
		...
		public static function LinerEase (t:Number, b:Number, c:Number, d:Number):Number {
			return c*t/d + b;
		}
		...
		public static function SinEaseOut (t:Number, b:Number, c:Number, d:Number):Number {
			return c * Math.sin(t/d * 1.5707963268) + b;
		}
		...
		public static var  MOVE = easeOut;
		
		public static var  MARKER_DRAG = BounceEaseOut;
